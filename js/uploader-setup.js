(function($) {
    ...
    $(function() {
        ...
        var createRowFromUploadItem = function(ui) {
            ...
            $('<div/>').append($preview).appendTo($row); // Preview
            $('<div/>').text(ui.file.name).appendTo($row); // Filename
            $('<div/>').text(Math.round(ui.file.size / 1024) + ' KB').appendTo($row); // Size in KB
            $('<div/>').append($pbWrapper).appendTo($row); // Status
            $('<div/>').append($cancelBtn).appendTo($row); // Cancel button
            return $progressBar;
        };
