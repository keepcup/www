...
$deletePosition =$_POST['deletePosition'];
	switch($table){
		case 'mainslider':
			...
			break;
		case 'photography':
			$select = $db->prepare("SELECT id,img,img_preview FROM photography WHERE position=?");
			$select->execute(array($deletePosition));
			$select_row = $select->fetch();
			unlink($select_row['img']);
			unlink($select_row['img_preview']);
		...
		case 'gallery':
			if(!empty($_POST['gallerydeleteflag'])){
				$id = $_POST['id'];
				$select = $db->prepare("SELECT id,img,img_preview FROM gallery_img WHERE gallery_id = ?");
				$select->execute(array($id));
				$select_row = $select->fetchAll();

				foreach ($select_row as $key => $value) {
					unlink($select_row[$key]['img']);
					unlink($select_row[$key]['img_preview']);
				}
